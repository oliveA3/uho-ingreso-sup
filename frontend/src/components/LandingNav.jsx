...
export default function LandingNav({ user, onLogout }) {
  return (
    <nav className="sticky top-0 z-40 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-200">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-5 py-5 sm:px-6">
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          {user?.rol?.name?.toLowerCase() === "super administrador" && (
            <Link to="/superadmin" className="text-sm font-medium text-slate-600 transition hover:text-sky-700">
              Panel Super Admin
            </Link>
          )}
        </div>
